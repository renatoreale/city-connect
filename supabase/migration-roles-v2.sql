-- ============================================================
-- CivicVoice — Migrazione v2: Nuovi ruoli + gerarchia geografica
-- Eseguire nell'SQL Editor di Supabase Dashboard
-- ============================================================

-- 1. Nuovi valori enum user_role
...
-- 2. Nuovo status "terminata" (squadra completa il lavoro, in attesa di conferma)
ALTER TYPE public.report_status ADD VALUE IF NOT EXISTS 'terminata' AFTER 'in_lavorazione';

-- 3. Tabelle geografiche
...
-- 4. Colonne geografiche in profiles
...
-- 5. Colonne assegnazione in reports
ALTER TABLE public.reports
  ADD COLUMN IF NOT EXISTS comune_id  UUID REFERENCES public.comuni(id),
  ADD COLUMN IF NOT EXISTS ufficio_id UUID REFERENCES public.uffici(id),
  ADD COLUMN IF NOT EXISTS squadra_id UUID REFERENCES public.squadre_lavoro(id);

-- 6. Migra ruoli esistenti ai nuovi valori
...
-- 7. Funzioni helper geografiche (SECURITY DEFINER = bypass RLS)
...
-- 9. RLS reports: aggiorna per nuovi ruoli
...
-- 10. RLS profiles: aggiorna per nuovi ruoli
...
-- 11. Aggiorna update_report_status per nuovi ruoli
...
-- 12. RLS status_history: aggiorna per nuovi ruoli
